...
void init_phi_prompt(LiquidCrystal_I2C *l, multiple_button_input *k[], char ** fk, int w, int h, char i); ///< This is the library initialization routine.
...
void simple_setup_6_buttons(LiquidCrystal_I2C * lcd, byte columns, byte rows, byte btn_u, byte btn_d, byte btn_l, byte btn_r, byte btn_b, byte btn_a); ///< Sets up the library with lcd object and 6 buttons
LiquidCrystal_I2C * simple_setup_phi_2(byte columns=20,byte rows=4);    ///< Set up the library for phi-2 shield, default 20X4
LiquidCrystal_I2C * simple_setup_phi_3(byte columns=20,byte rows=4);    ///< Set up the library for phi-3 shield, default 20X4
LiquidCrystal_I2C * get_lcd_ptr();                      ///< return pointer to the lcd
